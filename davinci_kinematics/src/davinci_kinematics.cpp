...
Eigen::Matrix4d Davinci_fwd_solver::fwd_kin_solve_(const Vectorq7x1& q_vec) {
    Eigen::Matrix4d A = Eigen::Matrix4d::Identity();
...
Eigen::Matrix4d Davinci_fwd_solver::fwd_kin_solve_approx_(const Vectorq7x1& q_vec) {
    Eigen::Matrix4d A = Eigen::Matrix4d::Identity();
    //%compute A matrix from frame i to frame i-1:
    Eigen::Matrix4d A_i_iminusi;
    Eigen::Matrix3d R;
...
Eigen::Vector3d Davinci_IK_solver::wrist_frame1_from_flange_wrt_rarm_mount(Eigen::Affine3d affine_flange_frame, Vectorq7x1 q_vec) {
...
Eigen::Vector3d Davinci_IK_solver::wrist_pt_wrt_frame1_of_flange_des_and_qs0(Eigen::Affine3d affine_flange_frame, Vectorq7x1 q_vec) {
    Eigen::Vector3d flange_z_axis_wrt_arm_mount;
    Eigen::Vector3d flange_origin_wrt_arm_mount;
...
Eigen::Vector3d Davinci_IK_solver::wrist_pt_from_flange_frame(Eigen::Affine3d affine_flange_frame) {
...
int Davinci_IK_solver::ik_solve_approx_wrt_torso(Eigen::Affine3d const& desired_flange_pose,std::vector<Vectorq7x1> &q_solns) {
    //convert desired_hand_pose into equiv w/rt right-arm mount frame:
...
int Davinci_IK_solver::ik_wristpt_solve_approx_wrt_torso(Eigen::Affine3d const& desired_hand_pose_wrt_torso,std::vector<Vectorq7x1> &q_solns) {
    //convert desired_hand_pose into equiv w/rt right-arm mount frame:
    //Eigen::Affine3d desired_hand_pose_wrt_arm_mount = transform_affine_from_torso_frame_to_arm_mount_frame(desired_hand_pose);
...
int Davinci_IK_solver::ik_solve_approx_wrt_torso_given_qs0(Eigen::Affine3d const& desired_hand_pose_wrt_torso,double q_s0, std::vector<Vectorq7x1> &q_solns) {
    //convert desired_hand_pose into equiv w/rt right-arm mount frame:
    //Eigen::Affine3d desired_hand_pose_wrt_arm_mount = transform_affine_from_torso_frame_to_arm_mount_frame(desired_hand_pose);
    Eigen::Affine3d desired_hand_pose_wrt_arm_mount = Affine_torso_to_rarm_mount_.inverse()*desired_hand_pose_wrt_torso;
...
int Davinci_IK_solver::ik_wrist_solve_approx(Eigen::Affine3d const& desired_flange_pose,std::vector<Vectorq7x1> &q_solns_123) // given desired hand pose, find all viable IK solns for wrist pt
{ 
...
//major fnc: samples values of q_s0 at resolution DQS0 to compute a vector of viable, approximate solutions to IK of desired_flange_pose
// for solutions of interest, can subsequently refine the precision of these with precise_soln_q123, etc.
int Davinci_IK_solver::ik_solve_approx(Eigen::Affine3d const& desired_flange_pose,std::vector<Vectorq7x1> &q_solns) // given desired hand pose, find all viable IK solns
{ 
  double q_s0_ctr = compute_qs0_ctr(desired_flange_pose);
  //cout<<"ik_solve_approx: q_s0_ctr = "<<q_s0_ctr<<endl;
  double dqs0 = DQS0; // search resolution on dq0
...
int Davinci_IK_solver::ik_solve_approx_elbow_orbit_from_flange_pose_wrt_torso(Eigen::Affine3d const& desired_flange_pose_wrt_torso,std::vector<std::vector<Eigen::VectorXd> > &path_options) {
...
int Davinci_IK_solver::ik_solve_approx_elbow_orbit_plus_qdot_s0_from_flange_pose_wrt_torso(
...
double  Davinci_IK_solver::precise_soln_q123(Eigen::Affine3d const& desired_flange_pose,Vectorq7x1 q123_approx, Vectorq7x1 &q123_precise) {
...
double  Davinci_IK_solver::compute_qs0_ctr(Eigen::Affine3d const& desired_flange_pose)
{ 
...
bool Davinci_IK_solver::compute_q123_solns(Eigen::Affine3d const& desired_flange_pose, double q_s0, std::vector<Vectorq7x1> &q_solns)
{ 
    q_solns.clear();
    Vectorq7x1 soln1_vec;
...
bool Davinci_IK_solver::solve_for_elbow_ang(Eigen::Vector3d w_wrt_1,double &q_elbow) {
...
bool Davinci_IK_solver::solve_for_humerus_ang(Eigen::Vector3d w_wrt_1,double q_elbow, double q_humerus[2]) {
...
bool Davinci_IK_solver::solve_for_s1_ang(Eigen::Vector3d w_wrt_1,double q_elbow, double q_humerus, double &q_s1) {
   //moment arm of humerus rotation is elbow offset plus displacement of wrist due to elbow rotation
...
bool Davinci_IK_solver::solve_spherical_wrist(Vectorq7x1 q_in,Eigen::Matrix3d R_des, std::vector<Vectorq7x1> &q_solns) {
...
bool Davinci_IK_solver::update_spherical_wrist(Vectorq7x1 q_in,Eigen::Matrix3d R_des, Vectorq7x1 &q_precise) {
...
bool Davinci_IK_solver::improve_7dof_soln(Eigen::Affine3d const& desired_flange_pose_wrt_arm_mount, Vectorq7x1 q_in, Vectorq7x1 &q_7dof_precise) {
