...
cv::Mat image, image_display,image_display_right,image_markup;
...
void color_fit_markup(double color_tol) {
    ...
    for (int y =0; y< height; y++)
        for (int x = 0; x< width; x++)   {
            image_markup.at<cv::Vec3b>(x, y)[0] = 0;
            image_markup.at<cv::Vec3b>(x, y)[1] = 0;
            image_markup.at<cv::Vec3b>(x, y)[2] = 255;
        }    
